...
def process_plate(raw_file, output_folders):
    """
    Process each plate from the raw data file 

    Extracts individual plate run from the raw file, cleans and normalize data, calculate
    QC metrics, generates QC plots, and save each file to a base folder for the experiment run

    Parameters:
    - raw_file (Dataframe): Raw input file containing data for each plate run
    - output_folders (dict): Dictionary of output folder paths for saving CSV files 
    and plots

    Returns:
    - results (dict): Returns a summary of the calculated QC metrics for each plate.
    """
    
    # Identify each plate run 
    index_pairs = raw_indices(raw_file)

    # Assign an empty dictionary for the result summary to be stored in 
    results = {}
    ...
    for i, (start, end) in enumerate(index_pairs):
        try:
            ...
            # Clean and process numeric data 
            ...
            # Calculate QC metrics (Signal to Background and Z-prime)
            ...
            # Save control data to a CSV file
